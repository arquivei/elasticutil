...
func Test_parseAggregations(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expectedResponse:            ResponseAggregation{},
			expectedError:               errors.New("parseAggregations: parseAggregationData: failed to decode aggregation data from response"),
		},
		{
			name:                        "error - unknown aggregation type",
			elasticAggregationsResponse: `{"max_aggregation_name":{"test":1025000.0}}`,
			expectedResponse:            ResponseAggregation{},
			expectedError:               errors.New("parseAggregations: parseAggregationData: aggregation type is not supported: max_aggregation_name - map[test:1.025e+06]"),
		},
		{
			name:                        "error - invalid buckets content",
			elasticAggregationsResponse: `{"some_histogram_agg":{"buckets":["test"]}}`,
			expectedResponse:            ResponseAggregation{},
			expectedError:               errors.New("parseAggregations: parseAggregationData: failed to decode bucket aggregation data from response"),
		},
	}
