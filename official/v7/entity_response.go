...
func checkErrorFromResponse(response *esapi.Response) error {
	...
	if !ok {
		return errors.New("failed to decode error from response: " + responseBody["error"].(string))
	}
...
func parseAggregationData(aggName string, aggData interface{}) (interface{}, error) {
	...
	if !ok {
		return nil, errors.Errorf("%s: failed to decode aggregation data from response", op)
	}
	...
	if buckets, ok := aggMap["buckets"].([]interface{}); ok {
		...
		for _, bucket := range buckets {
			...
			if !ok {
				return nil, errors.Errorf("%s: failed to decode bucket aggregation data from response", op)
			}
			...
			for name, data := range bucketMap {
				...
				case "key":
					if key, ok := data.(string); ok && responseBucket.Key == "" {
						responseBucket.Key = key
					}
				default:
	...
	return nil, errors.Errorf("%s: aggregation type is not supported: %s - %v", op, aggName, aggData)
}
